...
/**
 * "Share influencer list" modal — owner-only affordance on the campaign page.
 * Generates public, account-less links to the influencers preview (the shortlist
 * with recommendation notes + proposed pricing), lists active links, and revokes
 * them individually.
 *
 * Mirrors ShareCampaignModal / the tracking ShareReportModal exactly, but targets
 * the SEPARATE CampaignShortlistShareLink token so this list — which exposes
 * pricing — revokes independently of the campaign brief link. The public URL is
 * composed here from the browser origin + token; the backend never returns a URL.
 */
...
function publicUrl(token: string): string {
  ...
  return `${origin}/campaigns/public/${token}/influencers`;
}
...
export function ShareInfluencersModal({
  token,
  campaignId,
  onClose,
}: {
  ...
  const load = useCallback(() => {
    setLoading(true);
    apiListShortlistShareLinks(token, campaignId)
      .then((l) => {
        setLinks(l);
        setError(null);
      })
  ...
  const generate = async () => {
    ...
    setError(null);
    try {
      const link = await apiCreateShortlistShareLink(token, campaignId);
  ...
  const revoke = async (id: string) => {
    ...
    setError(null);
    try {
      await apiRevokeShortlistShareLink(token, id);
  ...
  const copy = async (link: CampaignShareLink) => {
    const ok = await copyText(publicUrl(link.token));
    if (ok) {
      setCopiedId(link.id);
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-foreground">Share influencer list</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4 p-6">
          <p className="text-sm text-muted-foreground">
            Anyone with a link can view this campaign&apos;s shortlisted creators —
            with your recommendation notes and proposed pricing — no login required.
            Budget, payment, and applicant data are never shown. Links expire
            automatically; revoke one anytime to disable it immediately.
          </p>
          ...
          {loading ? (
            ...
            <div className="rounded-xl border border-dashed border-border py-8 text-center text-sm text-muted-foreground">
              No active links yet. Generate one to share this list.
            </div>
          ) : (
