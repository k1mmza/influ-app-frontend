...
/**
 * "Share Report" modal — owner-only (agency/brand) affordance shown on the
 * authenticated tracking report page. Generates public, account-less links to
 * the report, lists the campaign's active links, and revokes them individually.
 *
 * Multiple links per campaign are supported and each is independently revocable,
 * so a leaked link can be killed without breaking others already handed out.
 * The public URL is composed here from the browser origin + token; the backend
 * never returns a URL.
 */
...
import {
  ...
  apiListShareLinks,
  apiRevokeShareLink,
  type TrackingShareLink,
} from "@/lib/api";
import { cn } from "@/lib/utils";

function publicUrl(token: string): string {
  const origin = typeof window !== "undefined" ? window.location.origin : "";
  return `${origin}/tracking/public/${token}`;
}
...
export function ShareReportModal({
  ...
  const [links, setLinks] = useState<TrackingShareLink[]>([]);
  ...
  const load = useCallback(() => {
    setLoading(true);
    apiListShareLinks(token, campaignId)
      .then((l) => {
        setLinks(l);
        setError(null);
      })
  ...
  const generate = async () => {
    ...
    setError(null);
    try {
      const link = await apiCreateShareLink(token, campaignId);
  ...
  const revoke = async (id: string) => {
    ...
    setError(null);
    try {
      await apiRevokeShareLink(token, id);
  ...
  const copy = async (link: TrackingShareLink) => {
    try {
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-foreground">Share report</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4 p-6">
          <p className="text-sm text-muted-foreground">
            Anyone with a link can view this campaign&apos;s report — no login
            required. Links expire automatically; revoke one anytime to disable
            it immediately.
          </p>
          ...
          {loading ? (
            ...
            <div className="rounded-xl border border-dashed border-border py-8 text-center text-sm text-muted-foreground">
              No active links yet. Generate one to share this report.
            </div>
          ) : (
